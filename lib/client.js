window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TEXT_FIELDS = [
			...
			{ key: "model", label: "手动模式模型（provider::模型）" },
			{ key: "autoOrder", label: "自动链顺序覆盖（provider::模型, ...）" },
			{ key: "reasoning", label: "思考强度（auto=不传 | minimal | low | medium | high；搜索推荐 low）" },
		...
		function Card(props) {
			...
			const subscribe = (callback) => scope.subscribe(callback);
			...
			async function save() {
				...
				try {
					...
					for (const f of TEXT_FIELDS.concat(NUMBER_FIELDS)) {
						...
						const isNumber = NUMBER_FIELDS.some((n) => n.key === f.key);
						const currentResolved = value[f.key];
						...
						if (unchanged && !(f.key in userLayer)) continue;
						if (raw.length === 0) writes.push(scope.unset(f.key));
						else if (isNumber) writes.push(scope.set(f.key, parseInt(raw, 10)));
				...
				snapshot.status !== "ready"
					...
					: react.createElement(react.Fragment, null,
						...
						react.createElement("div", { style: { display: "flex", gap: "8px", alignItems: "center" } },
							react.createElement("button", { onClick: save, disabled: disabled, style: { cursor: "pointer", borderRadius: "8px", padding: "6px 14px", fontSize: "13px", border: "1px solid transparent", background: "var(--dsw-alias-brand-primary, #2f6feb)", color: "#fff" } }, busy ? "保存中…" : "保存"),
		...
		console.log("[wso] factory executed");
		const inject = ["slots", "settingsScope"];

		function apply(ctx) {
		try { __wsoApply(ctx); console.log("[wso] apply ok"); } catch (e) { console.error("[wso] apply FAILED:", e); throw e; } }
		function __wsoApply(ctx) {
