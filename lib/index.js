 ...
 * persisted, with this precedence:
 *
 *   1. the plugin's settings section (`web-search-openai`, editable in the
 *      DSH settings UI) — fields the user saved there win;
 *   2. the session workspace `.env` (hot-reloaded on every search);
 *   3. built-in defaults.
 *
 * .env vocabulary (the settings section mirrors it):
 *
...
export const SettingsSchema = z.object({
	mode: z.string().description('auto | manual'),
	model: z.string().description('manual-mode selection, "provider::model"'),
	autoOrder: z.string().description('comma list of "provider::model" overriding the auto chain'),
...
/**
 * Best-effort synchronous seed for the settings section so a fresh install
 * shows the workspace .env choices instead of blanks. Runtime resolution keeps
 * re-reading .env asynchronously; saved section fields take precedence.
 */
...
export function apply(ctx, config = {}) {
	...
	async function runSearch(query, signal) {
		...
		if (chain.length === 0) {
			throw new WebError('[openai-search] 没有可尝试的模型：DSH 模型配置为空，或未选择手动模型（settings / DSH_SEARCH_MODEL）', 'WEB_PROVIDER_UNAVAILABLE');
		}
	...
	console.log('[dsh-web-search-openai] registered: ctx.web provider "openai-responses" + tool "openai_web_search" + settings section.');
}
