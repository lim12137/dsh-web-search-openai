 ...
 *
 * .env vocabulary (mirrored by the settings section). NOTE: names MUST NOT
 * start with `DSH_` — the harness boot rejects any `DSH_*` name found in a
 * boot-time .env layer (invoking dir or $DSH_HOME), so this plugin spells
 * its variables `WEB_SEARCH_*`; the old `DSH_SEARCH_*` names still work when
 * exported as real environment variables.
 *
 *   WEB_SEARCH_CHAIN=parallel,openai  backend order
 *   WEB_SEARCH_MODE=auto|manual       openai backend: auto chain vs single model
 *   WEB_SEARCH_MODEL=                 manual selection ("provider::model")
 *   WEB_SEARCH_AUTO_ORDER=            auto chain override ("provider::model,...")
 *   WEB_SEARCH_XSEARCH=0|1            attach x_search to grok-class models
 *   WEB_SEARCH_REASONING=auto|minimal|low|medium|high
 *                                      Responses API reasoning effort; 'auto'
 *                                      (default) omits the field entirely
 *   WEB_SEARCH_FALLBACK_KEY=          manual fallback credentials for openai backend
...
export const SettingsSchema = z.object({
	...
	reasoning: z.string().description('Responses API reasoning effort: auto (omit field) | minimal | low | medium | high'),
...
const USER_AGENT = 'dsh-web-search-openai/0.6.2';
...
/** Anything other than an explicit level (incl. 'auto'/empty) omits the field. */
...
/** New spelling wins; the pre-rename `DSH_SEARCH_*` names stay as fallback. */
function envPick(env, name, legacyName) {
	const v = env[name];
	return v === undefined || v === null || String(v).length === 0 ? env[legacyName] : v;
}

function configFromEnv(env) {
	return {
		chain: splitList(str(envPick(env, 'WEB_SEARCH_CHAIN', 'DSH_SEARCH_CHAIN'), 'parallel,openai')),
		mode: envPick(env, 'WEB_SEARCH_MODE', 'DSH_SEARCH_MODE') === 'manual' ? 'manual' : 'auto',
		model: str(envPick(env, 'WEB_SEARCH_MODEL', 'DSH_SEARCH_MODEL'), ''),
		autoOrder: splitList(envPick(env, 'WEB_SEARCH_AUTO_ORDER', 'DSH_SEARCH_AUTO_ORDER')),
		xsearch: truthy(envPick(env, 'WEB_SEARCH_XSEARCH', 'DSH_SEARCH_XSEARCH')),
		reasoning: normalizeReasoning(envPick(env, 'WEB_SEARCH_REASONING', 'DSH_SEARCH_REASONING')),
		fallbackKey: str(envPick(env, 'WEB_SEARCH_FALLBACK_KEY', 'DSH_SEARCH_FALLBACK_KEY'), str(env.OPENAI_API_KEY, '')),
		fallbackBaseURL: str(envPick(env, 'WEB_SEARCH_FALLBACK_BASE_URL', 'DSH_SEARCH_FALLBACK_BASE_URL'), str(env.OPENAI_BASE_URL, '')),
		maxOutputTokens: positiveInt(envPick(env, 'WEB_SEARCH_MAX_OUTPUT_TOKENS', 'DSH_SEARCH_MAX_OUTPUT_TOKENS') ?? env.OPENAI_SEARCH_MAX_OUTPUT_TOKENS, 4000, 16),
		timeoutMs: positiveInt(envPick(env, 'WEB_SEARCH_TIMEOUT_MS', 'DSH_SEARCH_TIMEOUT_MS') ?? env.OPENAI_SEARCH_TIMEOUT_MS, 120000, 3000)
	};
}
...
function seedSectionFromWorkspaceEnv() {
	try {
		const cfg = configFromEnv(parseEnv(readFileSync('.env', 'utf8')));
		// SettingsSchema describes the raw composition shape: comma lists stay strings.
		// Passing the resolved arrays here makes the async registration fail silently.
		return {
			chain: cfg.chain.join(','),
			mode: cfg.mode,
			model: cfg.model,
			autoOrder: cfg.autoOrder.join(','),
			xsearch: cfg.xsearch === true,
			reasoning: cfg.reasoning || 'auto',
			fallbackApiKey: cfg.fallbackKey,
...
function sectionToConfig(section, envFallback) {
	...
	return {
		...
		reasoning: normalizeReasoning(str(s.reasoning, envFallback.reasoning)),
...
/**
 * Test seam: pure internals for node:test and standalone live checks.
 * Not part of the plugin contract; cordis only consumes apply/name/inject.
 */
...
export function apply(ctx, config = {}) {
	...
	installSettingsSection(ctx, WEB_SEARCH_OPENAI_SETTINGS_NAMESPACE, SettingsSchema, seedSectionFromWorkspaceEnv() ?? config, {
		validate(value) {
			...
			const r = String(value.reasoning ?? 'auto').trim().toLowerCase();
			if (r !== '' && r !== 'auto' && !REASONING_LEVELS.includes(r)) throw new Error('reasoning 仅支持 auto/minimal/low/medium/high');
		},
		setSource: (source) => {
	...
	async function runOpenaiBackend(cfg, query, signal, failures, state) {
		...
		for (const value of chain) {
			...
			const callCfg = { baseURL, apiKey, maxOutputTokens: cfg.maxOutputTokens, timeoutMs: cfg.timeoutMs, reasoning: cfg.reasoning };
			const combos = [];
	...
	// registerSearchProvider() through the ctx.web proxy loses the entry on this
// cordis build (generator-effect bodies never execute; every ctx.web access
// hands back a fresh traceable facade). Write into the real provider Map
// directly and clean up with a generator-form effect, which DOES honor the
// post-yield disposer.
	const webMap = ctx.web?.searchProviders;
	if (!(webMap instanceof Map)) {
		console.log('[dsh-web-search-openai] warning: ctx.web.searchProviders unavailable; search provider not registered');
	} else {
		if (webMap.has(PROVIDER_ID)) throw new WebError(`a web provider with id "${PROVIDER_ID}" is already registered`, 'WEB_DUPLICATE_PROVIDER');
		webMap.set(PROVIDER_ID, {
			id: PROVIDER_ID,
			available() {
		...
		ctx.effect(function* () {
			yield () => { webMap.delete(PROVIDER_ID); };
		}, 'web-search-openai: provider map entry');
	}
	...
	console.log('[dsh-web-search-openai] v0.6.2 registered: unified provider (chain default parallel->openai) + tool.');
}
